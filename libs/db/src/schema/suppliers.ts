...
import {
  pgTable, uuid, varchar, text, boolean, timestamp, date, bigint, integer, uniqueIndex, index, check,
...
export const purchaseOrderItems = pgTable('purchase_order_items', {
  ...
  batchId: uuid('batch_id').references((): AnyPgColumn => inventoryBatches.id, { onDelete: 'set null' }),
  notes: text('notes'),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
  ...
  qtyReceivedCheck: check('purchase_order_items_qty_received_check', sql`${t.quantityReceived} >= 0`),
  qtyReceivedLimitCheck: check('purchase_order_items_qty_received_limit_check', sql`${t.quantityReceived} <= ${t.quantityOrdered}`),
}));
