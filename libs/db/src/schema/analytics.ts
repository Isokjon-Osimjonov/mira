import {
  pgTable, date, text, uuid, integer, bigint, primaryKey, index,
...
export const dailySalesSummary = pgTable('daily_sales_summary', {
  ...
  revenueKrw: bigint('revenue_krw', { mode: 'bigint' }).default(sql`0`).notNull(),
  cogsKrw: bigint('cogs_krw', { mode: 'bigint' }).default(sql`0`).notNull(),
  cargoKrw: bigint('cargo_krw', { mode: 'bigint' }).default(sql`0`).notNull(),
  couponDiscountKrw: bigint('coupon_discount_krw', { mode: 'bigint' }).default(sql`0`).notNull(),
  orderCount: integer('order_count').default(0).notNull(),
  refundCount: integer('refund_count').default(0).notNull(),
  refundedRevenueKrw: bigint('refunded_revenue_krw', { mode: 'bigint' }).default(sql`0`).notNull(),
}, (t) => ({
