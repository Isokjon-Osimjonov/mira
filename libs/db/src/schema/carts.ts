import {
  pgTable, uuid, text, timestamp, integer, bigint, uniqueIndex, index, check,
...
export const carts = pgTable('carts', {
  ...
  customerId: uuid('customer_id').unique().notNull().references(() => customers.id, { onDelete: 'cascade' }),
...
export const cartItems = pgTable('cart_items', {
  ...
  priceSnapshot: bigint('price_snapshot', { mode: 'bigint' }).default(sql`0`).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
}, (t) => ({
