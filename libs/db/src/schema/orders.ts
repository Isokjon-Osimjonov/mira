...
export const orders = pgTable('orders', {
  ...
  subtotal: bigint('subtotal', { mode: 'bigint' }).default(sql`0`).notNull(),
  discountAmount: bigint('discount_amount', { mode: 'bigint' }).default(sql`0`).notNull(),
  cargoFee: bigint('cargo_fee', { mode: 'bigint' }).default(sql`0`).notNull(),
  ...
  paymentVerifiedBy: uuid('payment_verified_by').references(() => adminUsers.id, { onDelete: 'set null' }),
  ...
  paymentConfirmedBy: uuid('payment_confirmed_by').references(() => adminUsers.id, { onDelete: 'set null' }),
  ...
  deliveryFeeCharged: bigint('delivery_fee_charged', { mode: 'bigint' }).default(sql`0`).notNull(),
...
export const orderItems = pgTable('order_items', {
  ...
  cargoFeeSnapshot: bigint('cargo_fee_snapshot', { mode: 'bigint' }).default(sql`0`).notNull(),
  ...
  unitPriceSnapshotCheck: check('order_items_unit_price_snapshot_check', sql`${t.unitPriceSnapshot} > 0`),
  subtotalSnapshotCheck: check('order_items_subtotal_snapshot_check', sql`${t.subtotalSnapshot} >= 0`),
}));
