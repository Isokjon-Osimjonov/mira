import {
  pgTable, uuid, varchar, text, integer, bigint, boolean, timestamp, jsonb, index, uniqueIndex, check,
...
export const productRegionalConfigs = pgTable('product_regional_configs', {
  ...
  productRegionIdx: uniqueIndex('product_regional_configs_product_region_idx').on(t.productId, t.regionCode),
  productIdIdx: index('product_regional_configs_product_id_idx').on(t.productId),
  regionCodeCheck: check('product_regional_configs_region_code_check', sql`${t.regionCode} IN ('UZB', 'KOR')`),
  currencyCheck: check('product_regional_configs_currency_check', sql`${t.currency} IN ('KRW')`),
  retailPriceCheck: check('product_regional_configs_retail_price_check', sql`${t.retailPrice} > 0`),
  wholesalePriceCheck: check('product_regional_configs_wholesale_price_check', sql`${t.wholesalePrice} > 0`),
  priceCompareCheck: check('product_regional_configs_price_compare_check', sql`${t.wholesalePrice} <= ${t.retailPrice}`),
  minWholesaleQtyCheck: check('product_regional_configs_min_wholesale_qty_check', sql`${t.minWholesaleQty} >= 1`),
  minOrderQtyCheck: check('product_regional_configs_min_order_qty_check', sql`${t.minOrderQty} >= 1`),
}));
