            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/50">
              ...
              <div className="flex gap-1">
                {notifications.length > 0 && (
                  <button onClick={() => clearMutation.mutate()} className="text-[11px] text-muted-foreground hover:text-red-500 transition-colors">
                    Tozalash
                  </button>
                )}
              </div>
                ...
                notifications.map((notif: any) => {
                  ...
                  return (
                    <button key={notif.id} onClick={() => handleNotifClick(notif)}
                      className={cn('w-full flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors text-left border-b border-border/30 last:border-0', !notif.isRead && 'bg-blue-50/30')}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1">{formatRelative(notif.createdAt)}</p>
                      </div>
                    </button>
                  )
                })
