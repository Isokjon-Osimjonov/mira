    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-xl border-[0.5px] border-border overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState message="Loglar topilmadi" description="Hozircha hech qanday amal bajarilmagan" />
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/50 bg-gray-50/80">
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground w-40">Vaqt</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground w-48">Admin</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground w-64">Amal</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground min-w-[200px]">Tafsilotlar</th>
                  </tr>
                </thead>
                  ...
                  {logs.map((log: any) => {
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-gray-50/60 transition-colors">
                        ...
                        <td className="px-4 py-3 align-top">
                          <p className="text-sm font-medium text-gray-900 line-clamp-1">{log.adminName || 'Tizim'}</p>
                          <p className="text-[10px] text-muted-foreground font-mono">{log.ipAddress}</p>
                        </td>
                        <td className="px-4 py-3 align-top">
                          <div className="flex items-start gap-2">
                            <Icon className="h-4 w-4 text-gray-400 mt-0.5 shrink-0" strokeWidth={1.5} />
                            <div>
                              ...
                              {log.entityType && (
                                <p className="text-[11px] text-muted-foreground">
                                  {log.entityType}: <span className="font-mono">{log.entityId?.slice(0,8)}</span>
                                </p>
                              )}
                            </div>
                          </div>
