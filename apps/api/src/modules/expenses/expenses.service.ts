...
import type { CreateExpenseCategoryDto, UpdateExpenseCategoryDto, CreateExpenseDto, UpdateExpenseDto } from './expenses.schema'
...
export async function updateExpenseCategory(id: string, data: UpdateExpenseCategoryDto) {
  const [category] = await db.select().from(expenseCategories).where(eq(expenseCategories.id, id)).limit(1)
  if (!category) throw { status: 404, code: 'EXPENSE_CATEGORY_NOT_FOUND', message: 'Kategoriya topilmadi' }
  ...
  const [updated] = await db.update(expenseCategories).set(updates).where(eq(expenseCategories.id, id)).returning()
  return updated
}
...
export async function deleteExpenseCategory(id: string) {
  const [category] = await db.select().from(expenseCategories).where(eq(expenseCategories.id, id)).limit(1)
  if (!category) throw { status: 404, code: 'EXPENSE_CATEGORY_NOT_FOUND', message: 'Kategoriya topilmadi' }
  if (category.isSystem) throw { status: 400, code: 'EXPENSE_CATEGORY_IS_SYSTEM', message: 'Tizim kategoriyasini o\'chirib bo\'lmaydi' }

  const [expenseCountRes] = await db.select({ count: count() }).from(expenses).where(eq(expenses.categoryId, id))
  if (Number(expenseCountRes?.count || 0) > 0) {
    throw { status: 400, code: 'EXPENSE_CATEGORY_IN_USE', message: 'Bu kategoriyada xarajatlar mavjud. O\'chirib bo\'lmaydi.' }
  }
...
export async function getExpenses(query: { page?: number, limit?: number, categoryId?: string, dateFrom?: string, dateTo?: string }) {
  ...
  const [countRes] = await db.select({ count: count(), totalAmount: sql<number>`SUM(amount_krw)`.mapWith(Number) }).from(expenses).where(where)
  ...
  return {
    ...
    meta: { page, limit, total, hasNext: offset + limit < total, hasPrev: page > 1, totalAmountKrw }
  }
}
...
export async function getExpenseById(id: string) {
  ...
  const [category] = await db.select().from(expenseCategories).where(eq(expenseCategories.id, expense.categoryId)).limit(1)
