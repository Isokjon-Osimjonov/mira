...
import {
  telegramChannels, telegramPosts, telegramPostChannels, products
} from '@mira/db'
import { eq, and, sql, desc, asc, isNull, inArray, count } from 'drizzle-orm'
import { bot } from '../../bot/bot'
import { env } from '../../config/env'
import type {
  CreateChannelDto, UpdateChannelDto, CreatePostDto, UpdatePostDto
...
export async function getChannels(query: { isActive?: boolean }) {
  ...
  const items = await db
    .select({
      ...
      postCount: sql<number>`(SELECT COUNT(*) FROM telegram_post_channels WHERE channel_id = ${telegramChannels.id} AND status = 'SENT')`.mapWith(Number)
    })
...
export async function createChannel(data: CreateChannelDto, adminId: string) {
  // Check unique chatId
  const [existing] = await db.select().from(telegramChannels).where(eq(telegramChannels.chatId, data.chatId)).limit(1)
  if (existing) throw { status: 409, code: 'CHANNEL_DUPLICATE', message: 'Ushbu chat ID allaqachon ro\'yxatdan o\'tgan' }

  // Test bot access
  try {
    await bot.api.getChat(data.chatId)
  } catch (err) {
    throw { status: 400, code: 'CHANNEL_BOT_NOT_ADMIN', message: 'Bot bu kanalga kira olmaydi. Botni admin qiling.' }
  }

  const [created] = await db.insert(telegramChannels).values({
    ...data,
    addedBy: adminId
  }).returning()
...
export async function updateChannel(id: string, data: UpdateChannelDto) {
  const [updated] = await db.update(telegramChannels).set({ ...data, updatedAt: new Date() }).where(eq(telegramChannels.id, id)).returning()
...
export async function testChannel(id: string) {
  const [channel] = await db.select().from(telegramChannels).where(eq(telegramChannels.id, id)).limit(1)
  ...
  try {
    ...
    throw { status: 400, code: 'CHANNEL_BOT_NOT_ADMIN', message: 'Xabar yuborishda xatolik yuz berdi. Bot kanalda adminligini tekshiring.' }
  }
}

// ─── Posts ───────────────────────────────────────────────────────────────

export async function getPosts(query: { page?: number, limit?: number, status?: string, dateFrom?: string, dateTo?: string }) {
  ...
  if (query.dateFrom) where = and(where, sql`${telegramPosts.createdAt} >= ${new Date(query.dateFrom)}`)
  if (query.dateTo) where = and(where, sql`${telegramPosts.createdAt} <= ${new Date(query.dateTo)}`)

  const itemsQuery = await db
    .select({
      post: telegramPosts,
      channelCount: sql<number>`(SELECT COUNT(*) FROM telegram_post_channels WHERE post_id = ${telegramPosts.id})`.mapWith(Number),
    })
    .from(telegramPosts)
    .where(where)
...
export async function createPost(data: CreatePostDto, adminId: string) {
  ...
  const channels = await db.select().from(telegramChannels).where(and(inArray(telegramChannels.id, data.channelIds), eq(telegramChannels.isActive, true)))
  if (channels.length === 0) throw { status: 400, message: 'Hech bo\'lmaganda bitta faol kanalni tanlang' }

  return await db.transaction(async (tx) => {
    const scheduledAt = data.scheduledAt ? new Date(data.scheduledAt) : null
    const isImmediate = !scheduledAt || scheduledAt <= new Date()

    const [newPost] = await tx.insert(telegramPosts).values({
      title: data.title,
...
export async function updatePost(id: string, data: UpdatePostDto) {
  const [post] = await db.select().from(telegramPosts).where(eq(telegramPosts.id, id)).limit(1)
  if (!post) throw { status: 404, code: 'POST_NOT_FOUND', message: 'Post topilmadi' }
  if (post.status === 'SENT') throw { status: 400, code: 'POST_ALREADY_SENT', message: 'Yuborilgan postni tahrirlab bo\'lmaydi' }
  ...
  return await db.transaction(async (tx) => {
    ...
    const [updated] = await tx.update(telegramPosts).set(updates).where(eq(telegramPosts.id, id)).returning()
...
export async function sendScheduledPosts(): Promise<number> {
  const posts = await db
    ...
    .from(telegramPosts)
    .where(and(
      eq(telegramPosts.status, 'SCHEDULED'),
      sql`${telegramPosts.scheduledAt} <= NOW()`
    ))
...
export async function deletePost(id: string) {
  const [post] = await db.select().from(telegramPosts).where(eq(telegramPosts.id, id)).limit(1)
  if (!post) throw { status: 404, code: 'POST_NOT_FOUND', message: 'Post topilmadi' }
  if (post.status === 'SENT') throw { status: 400, code: 'FORBIDDEN', message: 'Yuborilgan postni o\'chirib bo\'lmaydi' }

  const [deleted] = await db.update(telegramPosts).set({ status: 'ARCHIVED', updatedAt: new Date() }).where(eq(telegramPosts.id, id)).returning()
  return deleted
}
...
export async function sendPost(postId: string): Promise<void> {
  ...
  await db.update(telegramPosts).set({ status: 'SENT', sentAt: new Date() }).where(eq(telegramPosts.id, postId))
  ...
  for (const chan of channels) {
    try {
      ...
      await db.update(telegramPostChannels).set({
        status: 'SENT',
        sentAt: new Date(),
