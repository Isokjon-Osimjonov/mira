...
import {
  updateCustomerNotesSchema, blockCustomerSchema, assignCouponSchema
...
export async function getCustomers(req: Request, res: Response) {
  try {
    ...
    const data = await service.getCustomers({
      page, limit, search, region, isActive, isVerified, dateFrom, dateTo, sort, includeDeleted
    })
...
export async function updateNotes(req: Request, res: Response) {
  try {
    ...
    if (e.name === 'ZodError') return res.status(400).json({ data: null, error: { message: 'Xato', code: 'VALIDATION_ERROR' } })
    return res.status(e.status ?? 500).json({ data: null, error: { message: e.message, code: e.code ?? 'INTERNAL_ERROR' } })
  }
}
